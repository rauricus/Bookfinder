...
def ocr_onImage(image, east_model, debug=0, languages=None):
    ...
    # Sort boxes by position and get structure information for visualization
...
def showColumnRowStructure(image, structure_info):
    """
    Visualizes the detected column and row structure on the input image.
    
    Args:
        image: Input image to draw on
        structure_info: Dictionary containing structure information from TextRegionSorter
    ...
    total_columns = structure_info.get('total_columns', 0)
    columns = structure_info.get('columns', [])
    column_boundaries = structure_info.get('column_boundaries', [])
